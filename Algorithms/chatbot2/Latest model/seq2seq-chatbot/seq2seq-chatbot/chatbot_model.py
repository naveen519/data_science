...
class ChatbotModel(object):
    ...
    def __init__(self,
        ...
        if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
            ...
            self.sampling_temperature = tf.placeholder(tf.float32, name = "sampling_temperature") # Sampling temperature is to get more diverse samples, through stratified sampling
    ...
    def validate_batch(self, inputs, targets, input_sequence_length, target_sequence_length, metric = "loss"):
        ...
        Args:
            ...
            metric: The desired validation metric. Currently only "loss" is supported. "accuracy", "bleu", etc. are
            other common validation metrics that can be used.
        """
    ...
    def _build_encoder(self, encoder_embedded_input):
        ...
        if self.model_hparams.use_bidirectional_encoder:
            ...
            encoder_outputs, encoder_state = tf.nn.dynamic_rnn(
                                                    ...
                                                    inputs = encoder_embedded_input,
                                                    dtype = tf.float32,
                                                    swap_memory=True) # swap_memory allows swapping of memory from GPU to CPU. This expands the GPU memory, which helps in storing gradients of longer sentences
    ...
    def _build_attention_decoder_cell(self, encoder_outputs, encoder_state, batch_size):
        ...
        if self.beam_width > 0:
            encoder_outputs = tf.contrib.seq2seq.tile_batch(encoder_outputs, multiplier = self.beam_width) # tile_batch enables replication of a parameter a specific 'multiplier' times. https://www.tensorflow.org/api_docs/python/tf/contrib/seq2seq/tile_batch
