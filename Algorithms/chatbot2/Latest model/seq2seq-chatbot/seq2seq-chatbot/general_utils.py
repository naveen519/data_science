...
import datetime
import platform # platform type - linux, windows, etc.
from shutil import copyfile # Copies the contents of a file named src to a file named dst
...
def initialize_session(mode):
    ...
    if mode == "train" and args.datasetdir:
        ...
        if not os.path.isdir(dataset_dir): # checks whether the path exists in the directory or not
        ...
        dataset_name = os.path.basename(dataset_dir) # returns the file name from the path
